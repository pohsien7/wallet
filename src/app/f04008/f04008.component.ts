...
import { F04008confirmComponent } from './f04008confirm/f04008confirm.component';
import { F04008Service } from './f04008.service';

@Component({
  selector: 'app-f04008',
  templateUrl: './f04008.component.html',
  styleUrls: ['./f04008.component.css', '../../assets/css/f04.css']
})
export class F04008Component implements OnInit {
  ...
  constructor(private fb: FormBuilder, public f04008Service: F04008Service, private datePipe: DatePipe, public dialog: MatDialog) { }
  ...
  getViewDataList() {
    if (this.isFieldEmpty()) {
      this.dialog.open(F04008confirmComponent, { data: { msgStr: '請選擇一項查詢!' } });
      return;
    } else {
      ...
      this.f04008Service.sendConsumer('consumer/f04008', formdata).then(data => {

        if (data.totalCount == 0) {
          this.clear();
          return this.dialog.open(F04008confirmComponent, { data: { msgStr: '查無資訊!' } });
        }
  ...
  //   const formdata: FormData = new FormData();
  //   formdata.append('value', JSON.stringify(jsonObj));
  //   this.f04008Service.sendConsumer('consumer/f04008Shop', formdata).then(data => {
  //     const dialogRef = this.dialog.open(F04008confirmComponent, { data: { msgStr: data.result } });
  ...
  //   const dialogRef = this.dialog.open(F04008confirmComponent, { data: { msgStr: "check", userId: userId} });
  //   dialogRef.afterClosed().subscribe(result => {
  //     this.clear();
  //   });
  // }
