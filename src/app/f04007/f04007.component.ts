...
import { F04007confirmComponent } from './f04007confirm/f04007confirm.component';
import { F04007Service } from './f04007.service';

@Component({
  selector: 'app-f04007',
  templateUrl: './f04007.component.html',
  styleUrls: ['./f04007.component.css', '../../assets/css/f04.css']
})
export class F04007Component implements OnInit {
  ...
  constructor(private fb: FormBuilder, public f04007Service: F04007Service, private datePipe: DatePipe, public dialog: MatDialog) { }
  ...
  getViewDataList() {
    if (1 != 1) {
      ...
      this.f04007Service.sendConsumer('consumer/f04007', formdata).then(data => {

        if (data.totalCount == 0) {
          this.clear();
          return this.dialog.open(F04007confirmComponent, { data: { msgStr: '未查詢到相關錢包，請填寫正確查詢資料!' } });
        }
  ...
  //   const formdata: FormData = new FormData();
  //   formdata.append('value', JSON.stringify(jsonObj));
  //   this.f04007Service.sendConsumer('consumer/f04007Shop', formdata).then(data => {
  //     const dialogRef = this.dialog.open(F04007confirmComponent, { data: { msgStr: data.result } });
  ...
  //   const dialogRef = this.dialog.open(F04007confirmComponent, { data: { msgStr: "check", userId: userId} });
  //   dialogRef.afterClosed().subscribe(result => {
  //     this.clear();
  //   });
  // }
